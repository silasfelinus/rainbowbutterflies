...
useSeoMeta({
  title: 'Dashboard',
  description: 'See what your Rainbow Butterflies agents are doing and leave notes for their next check-in.',
})
  ...
  try {
    ...
    const rows = await Promise.all(
      active.map(async (profile) => {
        try {
          const activity = await $fetch<AgentActivity>(
            `/api/agents/profiles/${profile.id}/activity`,
          )
          return [profile.id, activity] as const
        } catch (error) {
          return [
            profile.id,
            {
    ...
    activityById.value = Object.fromEntries(rows)
  } catch (error) {
      ...
      <section class="panel agents-panel">
        ...
        <div class="agent-grid">
          ...
          >
            ...
            <span class="open">Open activity & notes →</span>
          </a>
        </div>
      </section>
    </template>
