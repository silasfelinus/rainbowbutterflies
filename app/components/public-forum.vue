...
async function toggleUpvote() {
  ...
  if (!thread || !signedIn.value || voteBusy.value) return

  voteBusy.value = true
  threadError.value = ''
  const upvoted = !Boolean(thread.viewerHasUpvoted)

  try {
    const response = await $fetch<{
      success: boolean
      data: { upvoteCount: number; viewerHasUpvoted: boolean } | null
      message?: string
    }>(`/api/forum/threads/${thread.id}/upvote`, {
      method: 'PUT',
      body: { upvoted },
    })
    ...
    thread.upvoteCount = response.data.upvoteCount
    thread.viewerHasUpvoted = response.data.viewerHasUpvoted

    const summary = threads.value.find((row) => row.id === thread.id)
    if (summary) {
      summary.upvoteCount = response.data.upvoteCount
      summary.viewerHasUpvoted = response.data.viewerHasUpvoted
    }

    if (order.value === 'upvotes') {
    ...
    threadError.value = error instanceof Error ? error.message : 'Your vote could not be saved.'
  } finally {
  ...
  <div class="forum-browser">
    <div class="forum-intro">
      <div>
        ...
        <span>Public, non-mature threads are readable by everyone. Choose Recent, Top, or chronological order.</span>
      </div>
    ...
    <div class="forum-toolbar">
      ...
      <div class="order-toggle" aria-label="Thread order">
        ...
        <button type="button" :class="{ active: order === 'upvotes' }" @click="order = 'upvotes'">Top</button>
    ...
    <div class="forum-content" :class="{ reading: selectedThreadId }">
        ...
        <template v-else>
          ...
          >
            ...
            <div class="thread-meta">
              <span class="vote-count">▲ {{ thread.upvoteCount }}</span>
          ...
          <div class="load-more">
            <button v-if="nextCursor" type="button" class="action" :disabled="threadsLoading" @click="loadThreads(false)">
              {{ threadsLoading ? 'Loading…' : order === 'upvotes' ? 'Load more top threads' : 'Load older threads' }}
            </button>
      ...
      <aside v-if="selectedThreadId" class="reader" aria-live="polite">
        ...
        <div v-else-if="threadError && !threadDetail" class="state-card error reader-state">
          ...
          <article class="root-post">
            ...
            <div class="upvote-row">
              <button
                ...
                type="button"
                class="upvote-button"
                :class="{ active: threadDetail.thread.viewerHasUpvoted }"
                :disabled="voteBusy"
                :aria-pressed="Boolean(threadDetail.thread.viewerHasUpvoted)"
                @click="toggleUpvote"
              >
                ▲ {{ threadDetail.thread.upvoteCount ?? 0 }}
                <span>{{ threadDetail.thread.viewerHasUpvoted ? 'Upvoted' : 'Upvote' }}</span>
              </button>
              <a v-else class="sign-in-upvote" href="/login?returnTo=%2F%23commons">
                ▲ {{ threadDetail.thread.upvoteCount ?? 0 }} · Sign in to upvote
              </a>
              <small v-if="threadError">{{ threadError }}</small>
            </div>
          </article>
