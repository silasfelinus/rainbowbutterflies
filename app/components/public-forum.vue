...
const { data: authState } = await useFetch<AuthState>('/api/auth/me', {
  key: 'rainbow-forum-auth-me',
  server: true,
  default: () => ({ authenticated: false, user: null, expiresAt: null }),
})
...
const currentUserId = computed(() =>
  authState.value.authenticated ? authState.value.user.id : null,
)
...
function canGenerateFrom(post: ForumPost) {
  return Boolean(currentUserId.value && post.author.user?.id === currentUserId.value)
}

function generationUrl(post: ForumPost) {
  return `https://kindrobots.org/art?forumPost=${post.id}`
}
</script>
      ...
      <aside v-if="selectedThreadId" class="reader" aria-live="polite">
        ...
        <template v-else-if="threadDetail">
          <article class="root-post">
            ...
            <a
              v-if="canGenerateFrom(threadDetail.thread)"
              class="forum-generate-action"
              :href="generationUrl(threadDetail.thread)"
              target="_blank"
              rel="noopener noreferrer"
            >
              <span>
                <strong>Illustrate in Kind Robots</strong>
                <small>Uses your Kind Robots generation balance. Compute spending is not a donation.</small>
              </span>
          ...
          <div v-else class="replies">
            ...
            >
              ...
              <a
                v-if="canGenerateFrom(row.post)"
                class="forum-generate-action"
                :href="generationUrl(row.post)"
                target="_blank"
                rel="noopener noreferrer"
              >
                <span>
