...
The browser keeps three values in local storage only:

- `rb_seen=1`, a boolean indicating that this browser has visited before;
- `rb_visit_day=YYYY-MM-DD`, the last UTC day counted;
- `rb_attribution`, containing a bucketed source/campaign pair plus a day-level expiry up to 30 days later.

These values are never cookies and therefore are not sent with ordinary HTTP requests. None is a random or stable visitor id. The app does not collect `document.referrer`, user-agent strings, full browsing URLs, IP addresses, browser fingerprints, or cross-site identifiers.
